...
?>
    <div class="form-field-wrap is_required">
        ...
        <div class="field-input">
            <?php
            ob_start();
            wp_editor($post_content, $field_id, $settings = array(
    ...
    ?>
        <div class="form-field-wrap">
            ...
            <div class="field-input">
                ...
                <input style="display: <?php if ($job_bm_create_account) echo 'block';
                                        else echo 'none'; ?>" placeholder="username" type="text" value="<?php echo esc_attr($job_bm_username); ?>" name="job_bm_username">
...
function job_bm_job_submit_form_company_logo()
{
    ...
    $job_bm_company_logo = isset($_POST['job_bm_company_logo']) ? esc_url_raw($_POST['job_bm_company_logo']) : job_bm_plugin_url . "assets/front/images/placeholder.png";
...
function job_bm_job_submit_data($post_data)
{
    ...
    if ($job_bm_account_required_post_job == 'yes' && !is_user_logged_in()) {
        ...
        $error->add('login',  sprintf(__(
            'ERROR: Please <a target="_blank" href="%s">login</a> to submit question.',
            'job-board-manager'
        ), $dashboard_page_url));
    }
